...
import { splitEmoji, EmojiImages, drawRichSprite, measureRichSprite } from './emoji.js';
...
export class CanvasDanmu {
  constructor(config) {
    ...
    this.bind(this.video, 'seeking', () => { this.beginSeek(); this.resetFrame(true); this.metrics.seeks++; this.cancel(); });
    this.bind(this.video, 'seeked', () => {
      this.buffering = false;
      this.resetFrame(true);
      clearTimeout(this.seekTask);
      // The native player synchronously calls clear/stop/start from SEEKED.
      // Restore after that lifecycle has settled, including when it stays paused.
      this.seekTask = setTimeout(() => {
        ...
        if (this._status !== 'closed') { this.frame(); this.wake(true); }
      }, 0);
    });
    ...
    this.bind(this.video, 'emptied', () => {
      ...
      this.buffering = false; this.clear();
    });
    ...
    this.bind(window, 'resize', () => { if (this.dpr !== (window.devicePixelRatio || 1)) this.resize(); });
  ...
  beginSeek() {
    ...
    this.seekCycle = true;
    clearTimeout(this.seekTask); this.seekTask = 0;
  }
  restoreWindow() {
  ...
  wake(immediate = false) {
    ...
    if (restoring) {
      // Yield between reconstruction slices without nested timer clamping.
  ...
  clear() {
    if (this.video.seeking) this.beginSeek();
    // Native clear/stop callbacks can arrive after SEEKED's task has finished.
    // Keep this video's reconstruction history until replacement or emptied.
  ...
  updateComments(comments, replace = true) {
    ...
    this.candidate = null;
    if (replace) { this.pending = []; this.pendingHead = 0; this.restoreQueue = []; this.restoreHead = 0; }
  ...
  resize() {
    ...
    if (this.renderer) { this.stage.width = 1; this.stage.height = 1; }
    else {
  ...
  flightProgress(c, time = this.now()) {
    // The compositor may lag the media clock after a rendering interruption.
    // A visible sprite owns its lifetime and collision position until it exits.
    return c.frozenProgress ?? (this._status === 'playing' && !this.video.paused && !c.staged ? this.renderer?.progress(c) : undefined) ?? this.progress(c, time);
  }
  retime(c, duration) {
    if (Math.abs(c.duration - duration) < 1e-8) return;
    const progress = c.frozenProgress ?? (this._status === 'playing' && !this.video.paused ? this.renderer?.progress(c) : undefined) ?? this.progress(c);
    c.duration = duration;
  ...
  frame() {
    ...
    while (this.restoreHead < this.restoreQueue.length) {
      ...
      // A seek reconstructs only comments still alive at the new media time;
      // it must never replay the old position's backlog or expired history.
      ...
      c.y = allocate.call({ media: { currentTime: c.time, playbackRate: 1 }, _: { width: this.width, height: 1e9, duration: c.duration, space: this.space } }, c);
      ...
      if (fits && !this.overlapsDuringFlight(c, this.active, time)) {
        ...
        c.staged = true;
        this.active.push(c);
        // Include bitmap upload in the slice budget; hidden sprites are not
        // revisited until the snapshot is ready to publish.
    ...
    if (this.restoreHead === this.restoreQueue.length) { this.restoreQueue = []; this.restoreHead = 0; }
    if (this.restoreBatchTime != null && !this.restoreQueue.length && !(items[this.position]?.time <= cutoff)) {
      ...
      if (items[this.position]?.time < publishTime) this.restoreBatchTime = publishTime;
        ...
        for (const c of this.active) if (c.staged) { c.staged = false; this.emit('bullet_start', c); }
      }
    }
  ...
  draw() {
    ...
    else canvasEngine.framing(this.stage);
  ...
  hitTest(event) {
    ...
    for (const c of this.active) {
      c.x = this.xForProgress(c, this.renderer?.progress(c) ?? this.progress(c));
    }
    const c = this.active.findLast(c => !c.staged && x >= c.x && x <= c.x + c.width && y >= c.y && y <= c.y + c.height);
    if (!c) return;
    try {
  ...
  releaseHover(resume = true) {
    ...
    if (c.el) {
      ...
      try { this.config.hooks?.bulletDetached?.(c.raw, c.el); }
      catch (error) { console.warn('[DY Danmaku] Hover cleanup failed', error); }
      finally { c.el.remove(); c.el = null; }
    }
    this.hovered = null;
    this.freezeId = null;
    this.manualFreeze = false;
